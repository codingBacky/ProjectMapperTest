...
public class CartServiceImpl implements CartService{
	...
	public void initCart(String email, OptionalInt itemNum, String size) {
		 int realItemNum = itemNum.orElse(0); // 기본값은 0으로 설정하거나 다른 값으로 지정
	    ...
	    //존재하면 개수만 1 늘려주고 없으면 detailCart를 추가해준다.
	        ...
	        List<Integer> cartNumList = mapper.getItemNums(cartNum);
	        boolean itemNumExists = false;

	        for (Integer checkCart : cartNumList) {
	...
	@Override
	public List<Integer> extractCartItemCnts(List<DetailCartVO> dvo) {
	        return dvo.stream()
